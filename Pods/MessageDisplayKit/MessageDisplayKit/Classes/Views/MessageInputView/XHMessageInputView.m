...
//        self.faceSendButton = button;
    ...
    if (!placeHolder) {
        placeHolder = NSLocalizedStringFromTable(@"Type a message...", @"MessageDisplayKitString", nil);
    }
        ...
        case XHMessageInputViewStyleFlat: {
            ...
            if (!inputBackgroundColor) {
                ...
                if (!inputViewBackgroundImageName) {
                    inputViewBackgroundImageName = @"input-bar-background-1";
                }
            }
            ...
            _inputTextView.frame = CGRectMake(textViewLeftMargin, 4.5f, width + 48, height);
//            _inputTextView.backgroundColor = [UIColor darkGrayColor];
            _inputTextView.backgroundColor = [UIColor colorWithRed:23.0/255.0 green:23.0/255.0 blue:23.0/255.0 alpha:23.0/255.0];
    ...
    if (self.allowsSendVoice) {
        ...
        [button setTitle:NSLocalizedStringFromTable(@"Hold To Talk", @"MessageDisplayKitString", nil) forState:UIControlStateNormal];
        [button setTitle:NSLocalizedStringFromTable(@"Release To Send", @"MessageDisplayKitString", nil)  forState:UIControlStateHighlighted];
        buttonFrame = CGRectMake(textViewLeftMargin-5, 0, width + 60, self.frame.size.height);
        button.frame = buttonFrame;
